Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("ProtocolCollectionDB_NET.ProtocolCollectionDB")> Public Class ProtocolCollectionDB
	'ProtocolBrandDBCollection.cls
	'
	...
	Private m_cProtocols As New Collection
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                Dim objProtocolDB As New ProtocolDB
                'create a new object

                'stuff all of the record's properties into the object
                objProtocolDB.Load((CType((.Tables(strStoredProc).Rows(i).Item("GuidID")), Guid).ToString())) '.Fields("guidID").Value))

                'add the object to the collection
                m_cProtocols.Add(objProtocolDB)
