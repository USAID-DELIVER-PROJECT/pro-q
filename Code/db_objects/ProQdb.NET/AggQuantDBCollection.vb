Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("AggQuantDBCollection_NET.AggQuantDBCollection")> Public Class AggQuantDBCollection
	'AggQuantdbcollection.cls
	'
	...
	Private m_cAggQuant As New Collection
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                Dim objAggQuantDB As New AggQuantDB

                'create a new object
                objAggQuantDB.SetQuantID(CType((.Tables(strStoredProc).Rows(i).Item("guidID")), Guid).ToString()) '.Fields("guidID").Value)
                objAggQuantDB.SetUseID(CType((.Tables(strStoredProc).Rows(i).Item("guidID_Use")), Guid).ToString()) '.Fields("guidID_Use").Value)
                objAggQuantDB.SetQuantName(.Tables(strStoredProc).Rows(i).Item("strName")) '.Fields("strName").Value)
                objAggQuantDB.SetScriptUseID(.Tables(strStoredProc).Rows(i).Item("id")) '.Fields("id").Value)
                'add the object to the collection
                m_cAggQuant.Add(objAggQuantDB)

                'release our ref to the obj
                'UPGRADE_NOTE: Object objAggQuantDB may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
                objAggQuantDB = Nothing

                'get the next record
            Next
        End With

        GetCollection = m_cAggQuant
