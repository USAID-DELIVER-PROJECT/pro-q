Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("ProQCollectionDB_NET.ProQCollectionDB")> Public Class ProQCollectionDB
	'ProtocolLevelCollectionDB.cls
	'
	...
	'recordset & sp functions
	Private m_objDBExec As New DBExec
	
	'the collection
	Private m_cDataObjects As Collection
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        'for each record
        Dim objRecord As New ProtocolLevelDB
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                'create an object to hold the record
                'load the object
                objRecord.Load((CType((.Tables(strStoredProc).Rows(i).Item("GuidID")), Guid).ToString())) '.Fields("guidID").Value))
                'add the object to the collection
                m_cDataObjects.Add(objRecord)
                'get the next record
            Next
        End With

        'return the collection
        GetCollection = m_cDataObjects
