Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("BrandCollectionDB_NET.BrandCollectionDB")> Public Class BrandCollectionDB
	'BrandDBCollection.cls
	'
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                ...
                Dim objBrandDB As New BrandDB

                'build the object
                objBrandDB.Load(CType((.Tables(strStoredProc).Rows(i).Item("GuidID")), Guid).ToString()) '.Fields("guidID").Value)

                'add the object to the collection
                m_cBrands.Add(objBrandDB)
                objBrandDB = Nothing
