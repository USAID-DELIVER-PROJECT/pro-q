Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("UseFundingDBCollection_NET.UseFundingDBCollection")> Public Class UseFundingDBCollection
	'UseFundingDBCollection.cls
	'
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With rst
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                Dim objUseFundingDB As New UseFundingDB
                'put the object in the collection


                'stuff all of the record's properties into the object
                objUseFundingDB.Load(CType((.Tables(strStoredProc).Rows(i).Item("GuidID")), Guid).ToString()) '.Fields("guidID").Value)

                'add the object to the collection
                m_cUseFundings.Add(objUseFundingDB)
                'UPGRADE_NOTE: Object objUseFundingDB may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
                objUseFundingDB = Nothing
