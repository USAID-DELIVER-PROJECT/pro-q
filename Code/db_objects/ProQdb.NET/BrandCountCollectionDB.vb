Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("BrandCountCollectionDB_NET.BrandCountCollectionDB")> Public Class BrandCountCollectionDB
	'BrandCountDBcollection.cls
	'
	...
	Private m_cBrandCount As New Collection
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                ...
                Dim objBrandCountDB As New BrandCountDB
                'create a new object
                objBrandCountDB.SetBrandCount(.Tables(strStoredProc).Rows(i).Item("BrandCount")) '.Fields("BrandCount").Value)

                'add the object to the collection
                m_cBrandCount.Add(objBrandCountDB)

                'release our ref to the obj
                'UPGRADE_NOTE: Object objBrandCountDB may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
                objBrandCountDB = Nothing

                'get the next record
            Next
        End With

        GetCollection = m_cBrandCount
