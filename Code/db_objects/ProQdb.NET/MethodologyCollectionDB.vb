Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("MethodologyCollectionDB_NET.MethodologyCollectionDB")> Public Class MethodologyCollectionDB
	'MethodologyDBCollection.cls
	'
	...
	'the collection of Methodologys
	Private m_cMethodologies As New Collection
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With rst
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                Dim objMethodologyDB As New MethodologyDB
                'put the object in the collection

                'stuff all of the record's properties into the object
                objMethodologyDB.Load(CType((.Tables(strStoredProc).Rows(i).Item("GuidID")), Guid).ToString()) '.Fields("guidID").Value)

                'add the object to the collection
                m_cMethodologies.Add(objMethodologyDB)
        ...
        GetCollection = m_cMethodologies
