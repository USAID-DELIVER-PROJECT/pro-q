Option Strict Off
Option Explicit On
<System.Runtime.InteropServices.ProgId("ResponseDBCollection_NET.ResponseDBCollection")> Public Class ResponseDBCollection
	'ResponseDBCollection.cls
    ...
    Public Function GetCollection(ByRef strStoredProc As String, ByRef aParams(,) As Object) As Collection
        ...
        With objRS
            For i = 0 To .Tables(strStoredProc).Rows.Count - 1 'Do Until .EOF
                Dim objDB As New ResponseDB
                'put the object in the collection

                'stuff all of the record's properties into the object
                objDB.Load(.Tables(strStoredProc).Rows(i).Item("lngTreeviewID")) '.Fields("lngTreeviewID").Value)
